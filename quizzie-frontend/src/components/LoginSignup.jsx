...
function LoginSignup({setCreatorData}) {
    ...
    const register = async () => {
        if (validation()) {
            try {
                const response = await axios.post('https://quizzie-backend-1-7ozy.onrender.com/register', {
    ...
    const login = async () => {
            if(loginValidation()) {
                try {
                    const response = await axios.post('https://quizzie-backend-1-7ozy.onrender.com/login', {
